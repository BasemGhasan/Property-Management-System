// ============================================================================
// AdminLayout — sidebar + top nav shell for the Admin Module.
// Identical responsive pattern to OwnerLayout; only nav items differ.
// ============================================================================

// Imports
import { useState, useCallback, type ReactNode } from "react";
import { useLocation, useNavigate } from "react-router";
import { Shield, Bell, LogOut, Menu, X, ChevronDown } from "lucide-react";
      ...
      <nav className="flex flex-1 flex-col gap-1 px-3 py-2">
        {ADMIN_NAV.map((item) => {
          ...
          const active =
            ...
            (item.to !== ADMIN_NAV[0].to && location.pathname.startsWith(item.to));
          return (
            <button
              key={item.to}
              onClick={() => go(item.to)}
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between gap-4 border-b border-slate-800 bg-slate-900/80 px-4 py-3 backdrop-blur sm:px-6">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-violet-600 text-xs text-white">AD</span>
              <span className="hidden text-sm text-slate-300 sm:block">Admin</span>
